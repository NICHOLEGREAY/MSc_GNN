...
class SpecialSpmmFunctionFinal(torch.autograd.Function):
    ...
    def forward(ctx, edge, edge_w, N, E, out_features):
        ...
        a = torch.sparse_coo_tensor(       #Constructs a sparse tensor in COO(rdinate) format with specified values at the given indices
            edge, edge_w, torch.Size([N, N, out_features]))
        b = torch.sparse.sum(a, dim=1)  # ijk-> sum over j
        ctx.N = b.shape[0]
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def __init__(self, num_nodes, in_features, out_features, nrela_dim, dropout, alpha, concat=True):
        ...
        self.a_2 = nn.Parameter(torch.zeros(size=(1, out_features)))    #W2 --gat absolute attention value of the triple
...
class SpGraphAttentionLayer_modified(nn.Module):
    """
    Sparse version GAT layer, similar to https://arxiv.org/abs/1710.10903
    """

    def __init__(self, num_nodes, in_features, out_features, nrela_dim, dropout, alpha, concat=True):
        super(SpGraphAttentionLayer_modified, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.num_nodes = num_nodes
        self.alpha = alpha
        ...
        self.a_2 = nn.Parameter(
            torch.zeros(size=(1, out_features)))  # W2 --gat absolute attention value of the triple
        nn.init.xavier_normal_(self.a_2.data, gain=1.414)
    ...
    def forward(self, scatter_entity, edge_continuous, edge_h, multi_gpu):
        ...
        if multi_gpu:
            self.a.data = self.a.data.to(edge_h.device)
            self.a_2.data = self.a_2.data.to(edge_h.device)
        ...
        edge_e = self.dropout(edge_e)
        edge_e = torch.where(torch.isnan(edge_e), torch.zeros_like(edge_e), edge_e)
        ...
        h_prime = torch.where(torch.isnan(h_prime), torch.zeros_like(h_prime), h_prime)
        assert not torch.isnan(h_prime).any()
        # h_prime: N x out
        
        h_prime = h_prime.div(e_rowsum)
        h_prime = torch.where(torch.isnan(h_prime), torch.zeros_like(h_prime), h_prime)  # replace  Nan by the entities not updated
        assert not torch.isnan(h_prime).any()
        if self.concat:
            # if this layer is not last layer,
            return F.elu(h_prime)
