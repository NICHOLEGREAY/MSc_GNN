...
def parse_args():
    ...
    args.add_argument("-outfolder", "--output_folder",
                      default="./checkpoints/ogb2/", help="Folder name to save the models.")
    args.add_argument("-outprint_file", "--outprint_file",
                      default="./outprint2.txt", help="File name to print information.")
    args.add_argument("-path_checkpoint", "--path_checkpoint",
                      default="./checkpoints/ogb2/trained_145.pth", help="last epoch")
    args.add_argument("-start_epoch", "--start_epoch",
                      default=146, type=int, help="start epoch num")

    # arguments for GAT
    args.add_argument("-b_gat", "--batch_size_gat", type=int,
                      default=262144, help="Batch size for GAT")  # 2^17 -- 131072
    ...
    args.add_argument("-layer_num", "--layer_num", type=int,
                      default=6, help="Number of layers of a GNN")
    args.add_argument("-device", "--GPU_device", type=int,
                      default=2, help="the primary GPU for training")
...
def train_gat(args):
    ...
    model_gat_modified.load_state_dict(torch.load(args.path_checkpoint), strict=False)
    ...
    if args.sampled ==1:
        ...
        sampled_indices = Corpus_.samlping_RW1(num_iters_per_epoch, f)  # takes around 5h
        np.savetxt(args.output_folder+'sampled_indiecs_np', sampled_indices)
        #sampled_indices = np.loadtxt((args.output_folder+'sampled_indiecs_np')

        print('RW Samlping finish! Time:{}\n'.format(time.time() - start_time))
        f.write('RW Sampling finish!  Time:{}\n'.format(time.time() - start_time))

    print("Number of residual epochs {}".format(args.epochs_gat - args.start_epoch ))
    f.write('Number of residual epochs {}\n'.format(args.epochs_gat - args.start_epoch ))

    # ____________ Trainning ___________
    for epoch in range(args.start_epoch, args.epochs_gat):
