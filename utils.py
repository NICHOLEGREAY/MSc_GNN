...
def compute_pure_source(batch_inputs,f):
    graph = {}
    all_triples =batch_inputs.to('cpu')
    field ={}
    for data in all_triples:
        source = data[0].data.item()
        target = data[2].data.item()
        value = data[1].data.item()
    ...
    for source in graph.keys():
        ...
        while not q.empty():
            ...
            if top in graph.keys():
                for target in graph[top].keys():
                    q.put(target)
                    distance[target] = distance[top]+1
                    if  source in  field.keys():
                        field[source] =max(distance[target], field[source])
                    else:
                        field[source] =distance[target]
    f.write("receptive field:\n")
    f.write(pickle.dumps(field))
    target_entity = np.unique(all_triples[:,2])
    source_entity = np.setdiff1d(all_triples[:,0], target_entity)
    entity = np.union1d(all_triples[:,0],all_triples[:,2])
    print("Out degree computed\n")  # graph {source: { target: value, .. }, ..}
    print('number of target entity:{}\n'.format(len(target_entity)))
    f.write('number of target entity:{}\n'.format(len(target_entity)))
    print('pure source ratio:{}\n'.format(len(source_entity)/len(entity)))
    f.write('pure source ratio:{}\n'.format(len(source_entity)/len(entity)))
